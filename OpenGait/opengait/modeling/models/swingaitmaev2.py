...
class SwinGaitmaev2(BaseModel):
    def __init__(self, cfgs, training): 
        self.T_max_iter = cfgs['trainer_cfg']['T_max_iter']
        super(SwinGaitmaev2, self).__init__(cfgs, training=training)
    ...
    def get_optimizer(self, optimizer_cfg):
        ...
        params_list = [
            {'params': [p for n, p in transformer_params if p.requires_grad and any(nd in n for nd in transformer_no_decay)], 'lr': backbone_lr, 'weight_decay': 0.}, 
            {'params': [p for n, p in transformer_params if p.requires_grad and not any(nd in n for nd in transformer_no_decay)], 'lr': backbone_lr, 'weight_decay': optimizer_cfg['weight_decay']}, 
            {'params': [p for p in self.decoder.parameters() if p.requires_grad], 'lr': decoder_lr, 'weight_decay': optimizer_cfg['weight_decay']},
            {'params': [p for p in self.decoder_edge.parameters() if p.requires_grad], 'lr': decoder_lr, 'weight_decay': optimizer_cfg['weight_decay']},
            {'params': [p for p in self.FCs.parameters() if p.requires_grad], 'lr': backbone_lr, 'weight_decay': optimizer_cfg['weight_decay']}, 
            {'params': [p for p in self.BNNecks.parameters() if p.requires_grad], 'lr': backbone_lr, 'weight_decay': optimizer_cfg['weight_decay']}, 
        ]
        for i in range(5): 
            if hasattr(self, 'layer%d'%i): 
                params_list.append(
                    {'params': [p for p in getattr(self, 'layer%d'%i).parameters() if p.requires_grad], 'lr': backbone_lr, 'weight_decay': optimizer_cfg['weight_decay']}
                )
