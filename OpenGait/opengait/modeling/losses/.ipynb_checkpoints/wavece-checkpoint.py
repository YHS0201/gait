...
class WaveCE(BaseLoss):
    def __init__(self, scale=2**4, label_smooth=True, eps=0.1, loss_term_weight=1.0, log_accuracy=False, l_min=-1.0,
                 l_max=1.0,
                 cycles=4,
                 cycle_iters=120000,):
        super(WaveCE, self).__init__(loss_term_weight)
        self.scale = scale
    ...
    def forward(self, logits, labels):
        ...
        base_losses = row_loss.mean(dim=0)
